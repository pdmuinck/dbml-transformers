...
public class DbmlToMySqlVisitor extends DbmlParserBaseVisitor<String> {
  ...
  private Map<String, Map<String, String>> info = new LinkedHashMap<>();
  ...
  public String visitTable(DbmlParser.TableContext ctx) {
    ...
    info.put(schemaTableName, new LinkedHashMap<>());
    String primaryKey = "";
  ...
  public String visitColumn(DbmlParser.ColumnContext ctx) {
    String schema_table = ((DbmlParser.TableContext) ctx.parent).schema_table_name().accept(this);
    if (enums.containsKey(ctx.type().getText())) {
      String column = String.format("`%s` ENUM (%s)", ctx.IDENTIFIER(0).getText(), enums.get(ctx.type().getText()).stream().map(e -> "'" + e + "'").collect(Collectors.joining(", ")));
      info.get(schema_table).putIfAbsent(ctx.IDENTIFIER(0).getText(), column);
      return column;
    }
    if (ctx.column_settings() != null) {
      ...
      if (settings.isEmpty()) {
        String column = String.format("`%s` %s", ctx.IDENTIFIER().get(0).getText(), ctx.type().accept(this));
        info.get(schema_table).putIfAbsent(ctx.IDENTIFIER(0).getText(), column);
        return column;
      } else {
        String column = String.format("`%s` %s %s", ctx.IDENTIFIER().get(0).getText(), ctx.type().accept(this), settings);
        info.get(schema_table).putIfAbsent(ctx.IDENTIFIER(0).getText(), column);
        return column;
      }
    } else {
  ...
  public String visitReference_short_form(DbmlParser.Reference_short_formContext ctx) {
    ...
    if (">".equals(relation)) {
      ...
      List<TerminalNode> identifier = ctx.IDENTIFIER();
      String table = ctx.IDENTIFIER(1).getText() + "_" + ctx.IDENTIFIER(3);
      info.putIfAbsent(table, new LinkedHashMap<>());
      info.get(table).putIfAbsent(ctx.IDENTIFIER(1) + "_" + ctx.IDENTIFIER(2), ctx.IDENTIFIER(1) + "_" + info.get(ctx.IDENTIFIER(1)).get(ctx.IDENTIFIER(2)));
      info.get(table).putIfAbsent(ctx.IDENTIFIER(1) + "_" + ctx.IDENTIFIER(2), ctx.IDENTIFIER(1) + "_" + info.get(ctx.IDENTIFIER(1)).get(ctx.IDENTIFIER(2)));
      statements.add(String.format("CREATE TABLE IF NOT EXISTS `%s` (`%s_%s`, `%s`)", table,
